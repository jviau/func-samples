...
internal readonly struct LogMessage
{
    ...
    public static LogMessage FromId(string id)
    {
        ...
        return id switch
        {
            nameof(Error_CannotRunFuncCli) or nameof(Strings.AZFW0100_Error_CannotRunFuncCli)
                => Error_CannotRunFuncCli,
            nameof(Error_ExtensionPackageConflict) or nameof(Strings.AZFW0101_Error_ExtensionPackageConflict)
                => Error_ExtensionPackageConflict,
            nameof(Warning_ExtensionPackageDuplicate) or nameof(Strings.AZFW0102_Warning_ExtensionPackageDuplicate)
                => Warning_ExtensionPackageDuplicate,
            nameof(Error_InvalidExtensionPackageVersion) or nameof(Strings.AZFW0103_Error_InvalidExtensionPackageVersion)
                => Error_InvalidExtensionPackageVersion,
            nameof(Warning_EndOfLifeFunctionsVersion) or nameof(Strings.AZFW0104_Warning_EndOfLifeFunctionsVersion)
                => Warning_EndOfLifeFunctionsVersion,
            nameof(Error_UsingLegacyFunctionsSdk) or nameof(Strings.AZFW0105_Error_UsingLegacyFunctionsSdk)
                => Error_UsingLegacyFunctionsSdk,
            nameof(Error_UnknownFunctionsVersion) or nameof(Strings.AZFW0106_Error_UnknownFunctionsVersion)
                => Error_UnknownFunctionsVersion,
            nameof(Error_UnsupportedTargetFramework) or nameof(Strings.AZFW0107_Error_UnsupportedTargetFramework)
                => Error_UnsupportedTargetFramework,
            nameof(Error_CustomFunctionPackageReferencesNotAllowed) or nameof(Strings.AZFW0108_Error_CustomFunctionPackageReferencesNotAllowed)
                => Error_CustomFunctionPackageReferencesNotAllowed,
            _ => new LogMessage(id),
        };
    }
    ...
    private static (LogLevel Level, string? Code) ParseId(string id)
    {
        ...
        int iteration = 0;
        while (index > 0 && iteration < 2)
        {
            iteration++;
            if (code is null && TryParseCode(id.AsSpan(0, index), out code))
            {
            }
            else if (level is null && TryParseLogLevel(id.AsSpan(0, index), out level))
            {
            }
